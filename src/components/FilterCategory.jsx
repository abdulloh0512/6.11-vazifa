...
function FilterCategory({ query: { query, setQuery } }) {
  ...
  return (
    <>
      <div
        className={`lg:!visible lg:!opacity-1 lg:!h-max md:${
          showCat ? "visible md:h-auto" : "invisible md:h-0"
        } ${showCat ? "visible h-auto" : "invisible h-0"} md:${
          showCat ? "opacity-1" : "opacity-0"
        } ${
          showCat ? "opacity-1" : "opacity-0"
        } mb-10 lg:ml-7 h-max  w-full  lg:w-2/12 min-w-48 text-xl font-medium dark:bg-grayshade-500 border border-grayshade-50 dark:border-grayshade-300 rounded-xl  pb-0 py-5 transition-all `}
      >
      ...
      <div
        ...
        className="lg:hidden md:block flex mb-5 cursor-pointer"
      >
